...
read -rp "Target install disk (default /dev/nvme0n1): " DISK
...
# Timezone defaults (override if desired)
...
Exec = /usr/local/bin/dracut-install.sh < /usr/lib/modules/*/pkgbase
Depends = dracut
NeedsTargets
HIN
...
# optional: make sure the new entry is first
ARCH_ID=\$(efibootmgr | awk '/Arch Linux/ { sub(/Boot/, ""); sub(/\\*/,""); print substr(\$1,1,4) }')
efibootmgr -o \${ARCH_ID}
